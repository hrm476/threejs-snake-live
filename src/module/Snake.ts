...
export const NODE_GEOMETRY = new RoundedBoxGeometry(0.9, 0.9, 0.9, 5, 0.1);
export const NODE_MATERIAL = new MeshStandardMaterial({
  color: 0xff470a,
});
...
export default class Snake extends EventDispatcher<SnakeEventMap> {
  ...
  mouthColor: number;
  mouth!: Mesh<RoundedBoxGeometry, MeshStandardMaterial, Object3DEventMap>;
  indexes: number[] = [];
  speedInterval = 240;
  body!: LinkedKList;
  ...
  stop() {
    this.isMoving && clearTimeout(this.isMoving);
    this.isMoving = null;
  }

  setDirection(keyCode: string) {
    let newDirection;
  ...
  update() {
    ...
    this.dispatchEvent({ type: "updated" } as never);
  }

  die() {
    let node: ListNode | null = this.body.head;
    ...
    this.init();
    this.dispatchEvent({ type: "die" } as never);
  }
  ...
  checkEntitiesCollision(entities: Array<Rock | Tree>) {
  ...
  updateIndexes() {
    ...
    let node: ListNode | null = this.body.end;
